...
function renderEvents() {
    ...
    featuredEvents.innerHTML = visibleEvents.slice(0, 3).map(event => `
        <article class="featured-card event-card" data-category="${categoryClass(event.category)}">
            <div class="event-image"${eventImageStyle(event.imageUrl)}>
            ...
            <div class="event-content">
                ...
                <div class="event-info"><span>◷ ${escapeHtml(event.time || "Time to be announced")}</span><span>⌖ ${escapeHtml(event.location || "Location to be announced")}</span></div>
    ...
    eventList.innerHTML = visibleEvents.slice(3).map(event => `
        <article class="list-event event-card" data-category="${categoryClass(event.category)}">
            <div class="list-event-image"${eventImageStyle(event.imageUrl)} aria-label="${escapeHtml(event.title || "Event")} image"></div>
            ...
            <div class="list-event-content"><span class="small-category ${categoryClass(event.category)}">${escapeHtml(event.category || "Event")}</span><h3>${escapeHtml(event.title || "Untitled event")}</h3><p>${escapeHtml(event.description || "No description provided.")}</p><div class="list-meta"><span>◷ ${escapeHtml(event.time || "Time to be announced")}</span><span>⌖ ${escapeHtml(event.location || "Location to be announced")}</span></div></div>
